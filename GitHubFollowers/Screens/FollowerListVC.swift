...
//        The New Way (IOS 15<=) for Generic / Default error Only:
...
//                self.presentGHFAlertOnMainThread(title: "Oops!", message: error.rawValue, buttonTitle: "ok")
//            }
//
...
//        }
    ...
    func updateUI(with followers: [Follower]) {
        ...
        if self.followers.isEmpty {
            let message = "This user has no followers yet 😒. Do You want to become the first one? 🤩"
        ...
        PersistenceManager.updateWith(favorite: favorite, actionType: .add) { [weak self] error in
            ...
            guard let error else {
                DispatchQueue.main.async {
                    self.presentGHFAlert(title: "Success 🥳", message: "You've favorited this one ⭐️", buttonTitle: "Yeah")
                }
                return
            }
            DispatchQueue.main.async {
                self.presentGHFAlert(title: "An Error Occurred 😧", message: error.rawValue, buttonTitle: "Ok")
            }
        }
    }
}
